...
export default function AchievementsSection() {
  ...
  return (
    <div ref={ref}>
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }} initial="hidden" animate={controls} className="space-y-4">
        {achievements.map((achievement, index) => (
          <motion.div key={achievement.id} custom={index} variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { type: "spring", damping: 12, stiffness: 100, delay: index * 0.05 } } }} className="group relative">
            <div className={`absolute -inset-0.5 bg-gradient-to-r ${getColorByCategory(achievement.category)} rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 blur-sm`}></div>

            <div className="relative bg-zinc-800 rounded-xl p-5 hover:bg-zinc-700/80 transition-all duration-300 shadow-lg hover:shadow-red-500/10 hover:scale-[1.01]">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div className="flex-1 mb-3 md:mb-0">
                  <h3 className="text-lg font-semibold text-amber-500 mb-2 group-hover:text-amber-400 transition-colors">{achievement.title}</h3>
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`text-xs px-3 py-1 rounded-full ${getBadgeColorByCategory(achievement.category)}`}>{achievement.organization}</span>
                    <span className="text-xs bg-red-500/20 text-red-400 px-3 py-1 rounded-full">{achievement.category}</span>
                ...
                {(achievement.profileLink || achievement.certificateLink) && (
                  <a href={achievement.profileLink || achievement.certificateLink} target="_blank" rel="noopener noreferrer" className="flex items-center text-cyan-500 hover:text-white transition-colors group/link">
              ...
              <p className="text-zinc-300 mt-3 text-sm">{achievement.description}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
