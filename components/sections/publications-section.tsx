...
export default function PublicationsSection() {
  ...
  return (
    <div ref={ref}>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate={controls} className="space-y-4">
        {publications.map((publication, index) => (
          <motion.div key={publication.id} custom={index} variants={itemVariants} className="group relative">
            ...
            <div className="relative bg-zinc-800 rounded-xl p-5 hover:bg-zinc-700/80 transition-all duration-300 shadow-lg hover:shadow-amber-500/10 hover:scale-[1.01]">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div className="flex-1 mb-3 md:mb-0">
                  <h3 className="text-lg font-semibold text-amber-500 mb-2 group-hover:text-amber-400 transition-colors">
                    {publication.title}
                  </h3>
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`text-xs px-3 py-1 rounded-full ${getJournalBadgeColor(publication.journal)}`}>
                      {publication.journal}
                    </span>
                ...
                <a
                  ...
                  className="flex items-center text-white hover:text-amber-500 transition-colors group/link"
                >
